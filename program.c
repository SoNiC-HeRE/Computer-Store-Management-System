...
main(){
	...
	comptag(0);
	system("color 0");
	puts("\tWhat exactly you want to do?");
	puts("	1. Purchase Components as a Customer?:");
	...
	switch (choice){
		...
		default : { comptag(0); system("color B1"); puts("\n\n\n\n\n\n\t\t\t\tTHANK YOU"); puts("\n\t\tFOR CHOOSING SoNiC's Service"); fclose(fptr); getch(); exit(0);}
	}
}
void Adminpermit(){
	system("color 0");
	...
	comptag(0);
	ADMINPANEL();
	printf("1. Add new laptop");
	printf("\n2. View available laptops");
...
{	fflush(stdin);
	...
	ADMINPANEL();
	FILE *tptr;
	printf("\n\t\t  Modification of existing laptop models \n");
	char id[30]; int a=0;
	char another='y';
...
{	fflush(stdin);
	...
	ADMINPANEL();
	FILE *tptr;
	printf("\n\t\t Deletion of existing laptop models that are out of stock \n");
	char id[30]; int a=0;
	char another='y';
	while(another=='y' || another=='Y')
	{	fflush(stdin);
...
int Access(){
	...
	scanf("%s",&admin);
	if(strcmpi(admin,"SoNiC")==0 || strcmpi(admin,"xyz")==0 || strcmpi(admin,"abc")==0)
...
{	system("color 0");
	...
	{	if(strcmpi(PC.laptopcompany,laptop)==0){
		...
		else { comptag(0); system("color B1"); puts("\n\n\n\n\n\n\t\t\t\tTHANK YOU"); puts("\n\t\tFOR CHOOSING SoNiC's Services"); fclose(fptr); getch(); exit(0);}
			}
